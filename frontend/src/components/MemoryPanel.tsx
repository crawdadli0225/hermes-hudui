  ...
  return (
    <div className="space-y-1.5">
      {entries.map((e: any, i: number) => (
        <div key={i} className="text-[12px] py-1.5 px-2" style={{ background: 'var(--hud-bg-panel)', borderLeft: '2px solid var(--hud-border)' }}>
          <div className="flex justify-between mb-0.5">
            <span className="uppercase tracking-wider text-[10px] font-bold" style={{ color: 'var(--hud-primary)' }}>
              {e.category}
            </span>
            <span className="text-[11px]" style={{ color: 'var(--hud-text-dim)' }}>{e.char_count}c</span>
          </div>
...
export default function MemoryPanel() {
  ...
  if (isLoading || !data) {
    return <Panel title="Memory" className="col-span-full"><div className="glow text-[12px] animate-pulse">Loading...</div></Panel>
  }
